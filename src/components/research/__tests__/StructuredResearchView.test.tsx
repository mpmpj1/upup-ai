...
const output: ResearchStructuredOutput = {
  ...
  bull_case: ['广告恢复快于预期', '回购强度维持'],
  bear_case: ['监管折价长期存在'],
  key_variables: ['广告恢复斜率', '游戏周期', '回购强度'],
  strongest_counterargument: '估值可能长期被流动性和监管折价压着。',
...
describe('StructuredResearchView', () => {
  it('renders thesis-first sections and Chinese content correctly', () => {
    ...
    expect(screen.getByText('腾讯更像现金流强但估值仍受折价的平台龙头。')).toBeInTheDocument();
    expect(screen.getByText('Bull Case')).toBeInTheDocument();
    expect(screen.getByText('Bear Case')).toBeInTheDocument();
    expect(screen.getByText('广告恢复斜率')).toBeInTheDocument();
    expect(screen.getByText('估值可能长期被流动性和监管折价压着。')).toBeInTheDocument();
    expect(screen.getByText('One-Line Takeaway')).toBeInTheDocument();
    expect(screen.getByText('Reuters coverage')).toBeInTheDocument();
