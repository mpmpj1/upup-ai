...
const KNOWN_ENTITIES: KnownEntity[] = [
  ...
  },
  {
    ...
    market_scope: 'us',
    search_terms: ['Alphabet', 'Google', 'GOOGL'],
    aliases: ['alphabet', 'google', 'googl', '谷歌', 'google母公司'],
  },
  {
    symbol: 'META',
    display_name: 'Meta',
    market_scope: 'us',
    search_terms: ['Meta', 'META'],
    aliases: ['meta', 'meta platforms', '脸书', 'facebook', 'fb'],
  },
  {
    symbol: '0700.HK',
    display_name: 'Tencent',
    market_scope: 'hk',
    search_terms: ['Tencent', '0700.HK', 'TCEHY'],
  ...
  },
  {
    ...
    market_scope: 'hk',
    search_terms: ['Alibaba', '9988.HK', 'BABA'],
    aliases: ['alibaba', '9988.hk', 'baba', '阿里巴巴', '阿里'],
  },
  {
    symbol: '1810.HK',
    display_name: 'Xiaomi',
    market_scope: 'hk',
    search_terms: ['Xiaomi', '1810.HK'],
    aliases: ['xiaomi', '1810.hk', '小米'],
  },
  {
    symbol: '3690.HK',
    display_name: 'Meituan',
    market_scope: 'hk',
    search_terms: ['Meituan', '3690.HK'],
    aliases: ['meituan', '3690.hk', '美团'],
  },
  {
    symbol: '1211.HK',
    display_name: 'BYD',
    market_scope: 'hk',
    search_terms: ['BYD', '1211.HK'],
    aliases: ['byd', '1211.hk', '比亚迪'],
  },
  {
    symbol: '300750.SZ',
    display_name: 'CATL',
    market_scope: 'cn',
    search_terms: ['CATL', 'Contemporary Amperex', '300750.SZ'],
  ...
  },
  {
    ...
    market_scope: 'cn',
    search_terms: ['Kweichow Moutai', '600519.SS'],
    aliases: ['moutai', '600519.ss', '贵州茅台', '茅台'],
  },
  {
    symbol: '601318.SS',
    display_name: 'Ping An',
    market_scope: 'cn',
    search_terms: ['Ping An', '601318.SS'],
    aliases: ['ping an', '601318.ss', '中国平安', '平安'],
  },
  {
    symbol: '002594.SZ',
    display_name: 'BYD',
    market_scope: 'cn',
    search_terms: ['BYD', '002594.SZ'],
    aliases: ['byd', '002594.sz', '比亚迪'],
  },
];
...
const FOLLOW_UP_PATTERNS = [
  ...
  /\bwhat about\b/i,
  /\bwhy\b/i,
  /\bhow does that change\b/i,
  /\bdoes that change\b/i,
  /\byou mentioned\b/i,
  /\bwhat would change your mind\b/i,
];

const FOLLOW_UP_BRIDGE_PATTERNS = [
  /^(那|这|如果|所以|那么|但|可是|然后|还有|对应地)/,
  /^(what about|how about|then|so|but)\b/i,
];

const MARKET_SCOPE_KEYWORDS = {
  us: ['美股', '纳斯达克', '标普', '道琼斯', 'us market', 'u.s. market'],
  hk: ['港股', '恒生', '恒指', '香港市场', 'h股'],
  cn: ['a股', 'A股', 'A-share', 'a-share', '沪深', '上证', '深证', '创业板', '科创板'],
} as const;
...
function inferMarketScopeFromQuery(query: string) {
  const raw = String(query || '');
  const normalized = normalizeText(raw);

  if (MARKET_SCOPE_KEYWORDS.cn.some((keyword) => raw.includes(keyword) || normalized.includes(keyword.toLowerCase()))) {
    return 'cn';
  }

  if (MARKET_SCOPE_KEYWORDS.hk.some((keyword) => raw.includes(keyword) || normalized.includes(keyword.toLowerCase()))) {
    return 'hk';
  }

  if (MARKET_SCOPE_KEYWORDS.us.some((keyword) => raw.includes(keyword) || normalized.includes(keyword.toLowerCase()))) {
    return 'us';
  }
...
function resolveRequestedMarketScope(
  requestedMarketScope: string | undefined,
  ...
  if (
    ...
    normalized.includes('ashare') ||
    normalized.includes('a-share')
  ) {
    return 'cn';
  }

  const inferredFromQuery = inferMarketScopeFromQuery(query);
...
function detectFollowUpSignal(
  query: string,
  entityHints: EntityHint[],
  continuity?: ResearchConversationContinuity,
) {
  if (!continuity?.has_prior_thesis) {
    return false;
  }

  if (matchesAny(FOLLOW_UP_PATTERNS, query)) {
    return true;
  }
  ...
  const shortBridgedFollowUp = query.length <= 24 && matchesAny(FOLLOW_UP_BRIDGE_PATTERNS, query);
  const hasQuestionMark = /[？?]$/.test(query);
  return shortBridgedFollowUp || (query.length <= 14 && hasQuestionMark);
}
...
export function classifyResearchTask(input: {
  ...
  const marketScope = resolveRequestedMarketScope(input.market_scope, entityHints, query);
  const reasons: string[] = [];
  const directActionRequest = matchesAny(DIRECT_ACTION_PATTERNS, query);
  const outOfScope = matchesAny(OUT_OF_SCOPE_PATTERNS, query);
  const cardRequest = matchesAny(CARD_REQUEST_PATTERNS, query);
  const eventSignal = matchesAny(EVENT_UPDATE_PATTERNS, query);
  const followUpSignal = detectFollowUpSignal(query, entityHints, continuity);
